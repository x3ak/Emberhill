...
function carveRiverFrom(rng: RandomFn, riverId: number, riverStart: Tile, grid: Grid): void {
    ...
    for (let j = 0; j < maxRiverLength; j++) {
        ...
        const lowestNeighbor = findRiverBedNextStep(rng, currentTile, grid) //findLowestNeighbor(currentTile, grid);
...
function findRiverBedNextStep(rng: RandomFn, tile: Tile, grid: Grid): Tile | null {
    ...
    const randomIndex = Math.floor(rng() * candidatePool.length);
