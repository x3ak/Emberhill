...
export default function TownFeatureList({setActiveSection, activeSection}: {
    ...
    const townFeatureList = ['storage', 'warmstone'].map(townFeatureId => {
        ...
        return (
            <li key={townFeatureId} className={sectionClassName} onClick={() => {
                setActiveSection({type: 'town_feature', feature: townFeatureId})
            }}>{townFeatureId}</li>
        )
    })
